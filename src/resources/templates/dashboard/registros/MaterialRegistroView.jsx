...
export function MaterialRegistroView() {
  ...
  // ✨ Se eliminó 'fechaPublicacion' de aquí
  ...
  useEffect(() => {
    const cargarMallaAcademica = async () => {
      ...
      try {
        ...
        console.error("Error cargando parámetros estructurales para materiales:", err);
  ...
  const handleRegistrarMaterial = async (e) => {
    ...
    if (!seleccion.idClase) {
      setMensaje({ texto: 'Debe seleccionar una Clase/Sesión válida para asociar el material.', tipo: 'error' });
      return;
    }
    ...
    // Payload enviado sin fecha (el servidor la calcula)
    const payload = {
    ...
    try {
      ...
      setMensaje({ texto: '¡El material de estudio se ha cargado y publicado con éxito en la sesión académica!', tipo: 'success' });
      ...
      console.error("Error registrando material semanal:", err);
      const errorMsg = err.response?.data?.error || 'Error de conexión con el servidor al almacenar el material.';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ background: 'var(--card-bg)', padding: '24px', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
        <h3 style={{ color: 'var(--text-main)', fontWeight: '600', marginBottom: '14px' }}>
          📂 Repositorio y Carga de Material Semanal
        </h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '14px' }}>
          ...
          <div>
            <label style={{ fontWeight: '500', fontSize: '0.85rem', display: 'block', marginBottom: '4px', color: 'var(--primary-color)' }}>Clase / Sesión</label>
      ...
      {/* RECUADRO DE MENSAJES OPERATIVOS */}
      {mensaje.texto && (
        <div style={{ 
          padding: '16px', paddingLeft: '20px', borderRadius: '8px', fontWeight: '500',
      ...
      {/* SECCIÓN 2: FORMULARIO DE ALTA */}
      <div style={{ background: 'var(--card-bg)', padding: '24px', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
        <h4 style={{ color: 'var(--text-main)', fontWeight: '600', marginBottom: '16px' }}>📝 Información del Material Didáctico</h4>
        
        <form onSubmit={handleRegistrarMaterial} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <label style={{ fontWeight: '500', display: 'block', marginBottom: '4px', fontSize: '0.9rem' }}>Título del Material</label>
            <input type="text" name="titulo" value={formMaterial.titulo} onChange={handleFormChange} required placeholder="Ej: Diapositivas de la Semana 4 - Ecuaciones Cuadráticas" style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1.5px solid var(--border-input)' }} />
          </div>

          <div>
            <label style={{ fontWeight: '500', display: 'block', marginBottom: '4px', fontSize: '0.9rem' }}>Descripción / Resumen explicativo</label>
            <textarea name="descripcion" value={formMaterial.descripcion} onChange={handleFormChange} rows="4" placeholder="Indique un breve resumen del material compartido o instrucciones de lectura obligatoria..." style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1.5px solid var(--border-input)', fontFamily: 'inherit' }}></textarea>
          </div>

          <div>
            <label style={{ fontWeight: '500', display: 'block', marginBottom: '4px', fontSize: '0.9rem' }}>Enlace de descarga del archivo / nube</label>
            <input type="url" name="urlArchivo" value={formMaterial.urlArchivo} onChange={handleFormChange} placeholder="https://onedrive.live.com/... o enlace de PDF adjunto" style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1.5px solid var(--border-input)' }} />
          </div>

          {/* 🗓️ Se eliminó completamente la sección visual del selector de fecha */}

          <div style={{ textAlign: 'right', marginTop: '10px' }}>
            <button type="submit" disabled={submitting || clasesFiltradas.length === 0 || loading} style={{ padding: '12px 32px', background: '#0284c7', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: '600', cursor: 'pointer', fontSize: '0.95rem' }}>
              {submitting ? 'Guardando Material...' : '💾 Publicar Material de Estudio'}
            </button>
          </div>
        </form>
      </div>
